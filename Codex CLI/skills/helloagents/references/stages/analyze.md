...
普通项目规则:
  ...
  调用 explorer 条件（满足任一时调用）:
    - 项目文件数 > 100
    - 目录深度 > 5
    - 需要跨多个模块定位文件

  调用 analyzer 条件（满足任一时调用）:
    - 依赖关系涉及 > 5 个模块
    - 需要深度代码结构分析
    - 技术风险评估复杂

判断结果:
  满足条件: 通过子代理执行，结果折叠后继续
  不满足条件: 主代理直接执行
...
降级处理:
  子代理调用失败时:
    - 主代理降级为直接执行
    - 在输出中标注 "[降级执行]"
    - 降级信息传递给后续 DESIGN 阶段，记录到 tasks.md 元数据
```
...
输出物: 项目上下文信息（技术栈、模块结构、质量问题、技术约束）供方案规划使用
