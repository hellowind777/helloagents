...
STAGE_ENTRY_MODE = DIRECT（~exec命令直接进入）:
  方案包来源: 扫描 plan/ 目录
  单个方案包: 自动选择
  多个方案包: 按G3场景内容规则（确认场景）输出，等待用户选择
...
步骤2 - 判断数量:
  ...
  多个方案包: 按G3场景内容规则（确认场景）输出方案包选择清单，等待用户选择

步骤3 - 验证方案包完整性:
  缺失文件: 按G3场景内容规则（错误场景）输出，流程终止
```
...
全授权命令（WORKFLOW_MODE=AUTO_FULL）:
  ...
  - 检查该方案包是否存在且完整
    ...
    - 不存在或不完整 → 按G3场景内容规则（错误场景）输出并停止
  - 忽略 plan/ 中的其他遗留方案包

交互确认模式/执行命令（STAGE_ENTRY_MODE=DIRECT）:
  - 扫描 plan/ 目录下所有方案包
  - 不存在方案包 → 按G3场景内容规则（错误场景）输出并停止
  - 方案包不完整 → 按G3场景内容规则（错误场景）输出并停止
  - 单个完整方案包 → 设置 CURRENT_PACKAGE，继续执行
  - 多个方案包 → 列出清单，等待用户选择
    - 用户输入有效序号(1-N) → 设置 CURRENT_PACKAGE，继续执行
    - 用户输入取消/拒绝 → 按G3场景内容规则（取消场景）输出，流程终止
    - 无效输入 → 再次询问
```
...
任务失败处理(状态更新为 [X]):
  - 记录错误信息（用于迁移前添加备注）
  - 保留错误信息在 tasks.md 中（帮助后续避免重复错误）
  - 继续执行后续任务
  - 所有任务完成后，如存在失败:
    - 交互确认模式/执行命令: 列出失败清单，询问用户决定
      - 用户选择继续 → 继续后续步骤
      - 用户选择终止 → 输出"已终止执行"，流程终止
...
```yaml
# 按 G10 注意力控制规则 + attention.md 执行

更新时机: 每次状态变更后（任务完成/失败/跳过）

更新内容:
  1. 更新 tasks.md 中对应任务的状态符号
  2. 更新 tasks.md 的 LIVE_STATUS 区域
  3. 追加执行日志（最近5条）

LIVE_STATUS 区域格式（在 tasks.md 中）:
  <!-- LIVE_STATUS_BEGIN -->
  状态: running | 进度: {完成数}/{总数} ({百分比}%) | 更新: {HH:MM:SS}
  当前: {正在执行的任务描述}
  <!-- LIVE_STATUS_END -->

目的: 外部监控可通过 tail -f 或 VS Code 实时查看进度
```
...
> 按 references/services/package.md "方案包生命周期管理" 中的迁移规则执行
> 脚本路径、存在性检查、错误恢复规则见 references/rules/tools.md
...
按 references/services/package.md "用户选择处理 - Overview类型方案包处理" 执行。
