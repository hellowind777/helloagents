...
各阶段调用规则:
  analyze 阶段:
    - explorer: 项目文件数 > 100 或首次分析
    - analyzer: 依赖关系涉及 > 5 个模块

  design 阶段:
    - analyzer (并行): 候选方案 ≥ 2 个
    - synthesizer: 多方案需要对比综合

  develop 阶段:
    - implementer: 单任务 > 50 行或涉及 ≥ 3 个文件
    - reviewer: 修改核心模块或涉及 EHRB
    - tester: 新增公共 API 或 tasks.md 要求测试
...
```yaml
执行规则: 按 G4 "改动型处理流程"执行（评分、追问循环、复杂度判定）
