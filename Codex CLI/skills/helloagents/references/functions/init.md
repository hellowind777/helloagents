...
**重建确认（知识库已存在且完整时）：** 按 G3 场景内容规则（确认）输出
...
- 空项目（无源代码文件） → 按 G3 场景内容规则（错误）输出，建议确认目录
- 权限不足（无法创建目录） → 按 G3 场景内容规则（错误）输出，建议检查权限
