...
命令: ~validate
类型: 范围选择类
功能: 验证知识库和方案包质量，必须选择验证范围（全部/仅知识库/仅方案包），只读操作
```
...
按 G3 场景内容规则（确认）输出，见"用户选择处理 - 验证范围选择"
```
...
参数:
  package-name: 可选，指定方案包名称；不指定则验证所有
输出: JSON 格式的验证结果
...
有问题时:
  按 G3 场景内容规则（确认）输出，见"用户选择处理 - 验证问题处理"
  [等待用户响应]
  用户选择后按对应选项处理
...
> 本章节定义 ~validate 命令需要用户确认的场景，供 G3 输出格式统一提取。
