...
命令: ~rollback
类型: 目标选择类
功能: 按照执行流程回滚变更，可选择回滚目标（版本/提交），触发 EHRB 时按 G2 处理流程执行
```
...
执行内容:
  - 优先使用 Git 历史，备选: 方案包历史、CHANGELOG
  - 最多显示5个版本，显示变更摘要和文件数
  - 警告回滚影响

按G3场景内容规则（确认场景）输出，见"用户选择处理 - 回滚版本选择"
```
...
完成后: 按G3场景内容规则（完成场景）输出回滚结果（含验收报告）
执行: 按 G6 状态重置协议执行
```
...
内容要素:
  - 目标版本: 将回滚到的版本信息
  - 影响范围: 将被覆盖的文件列表和变更内容
  - 风险说明: 按G3场景内容规则（警告场景）格式（风险类型、影响范围、风险等级、检测依据）
