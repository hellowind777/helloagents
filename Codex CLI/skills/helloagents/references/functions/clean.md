...
命令: ~clean
类型: 场景确认类
功能: 按照执行流程清理 plan/ 目录中的遗留方案包，列出方案包让用户选择清理范围
```
...
2. 评估深度: 轻量评估（无评分追问）
3. 清理操作需用户确认范围（全部/选择性）
4. 方案包迁移至 archive/ 而非直接删除
5. 迁移时标记状态为 skipped
</mode_adaptation>
...
无方案包时: 按G3场景内容规则（完成场景）输出提示，流程结束
...
完成后: 按G3场景内容规则（完成场景）输出清理结果（含验收报告）
执行: 按 G6 状态重置协议执行
```
...
- plan/ 目录不存在 → 按G3场景内容规则（完成场景）输出，提示无需清理
