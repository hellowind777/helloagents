...
命令生成检查（CRITICAL - Shell 命令执行前必须完成）:
  触发: 生成包含引号、变量、或嵌套调用的 Shell 命令时
  意图分析:
    1. 变量展开风险: 外层双引号内是否有 $var（会被提前展开为空）
    2. 嵌套复杂度: 引号嵌套是否超过1层（PowerShell→Python→字符串）
    3. 多行代码: 是否超过3行（\n 是字面字符，here-string 不能作为 -Command 参数值）
    4. 特殊字符: 是否包含中文路径、转义序列
  决策:
    全部无风险 → 直接执行，-Command 参数使用外层单引号
    任一有风险 → 改用临时文件方案（写入脚本 → 执行文件 → 删除）
```
...
**场景内容规则:** 从触发模块/类型的"用户选择处理"章节提取内容要素和选项
...
评估深度:
  ...
  轻量评估（场景确认类/范围选择类/目标选择类）:
    ...
    步骤2 - 安全分析: 按G2 EHRB检测
    步骤3 - 输出确认: 按G3 场景内容规则（需求确认-轻量评估）输出确认，等待用户选择
