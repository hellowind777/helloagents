...
test('CLI lifecycle covers standby, global, update, cleanup, and config preservation', () => {
  ...
  const grokMd = readText(join(home, '.grok', 'AGENTS.md'))
  ...
  const grokHooksText = JSON.stringify(readJson(join(home, '.grok', 'hooks', 'helloagents.json')))
  ...
  assert.match(grokHooksText, /helloagents-js notify inject --grok/)
  assert.match(grokHooksText, /helloagents-js guard --grok/)
  ...
  assert.equal(realTarget(join(home, '.codex', 'helloagents')), runtimeRoot)
  assert.ok(existsSync(join(cursorInstallRoot, '.cursor-plugin', 'plugin.json')))
  assert.ok(existsSync(join(cursorInstallRoot, 'hooks', 'hooks-cursor.json')))
  assert.notEqual(realTarget(cursorInstallRoot), cursorPluginRoot)
