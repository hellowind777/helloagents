...
test('workflow skill contracts stay aligned with command aliases and artifacts', () => {
  ...
  assert.match(helloagents, /`🔄 下一步` 必须写真实动作/)
  assert.match(helloagents, /不得把待执行事项包装成完成态/)
  ...
  assert.match(auto, /`🔄 下一步` 不能作为阶段交接或继续执行占位/)
  assert.match(auto, /必须写结构化 `turn-state` blocker/)
  assert.match(auto, /不得把“给出方案”“给出任务列表”“给出建议下一步”当作 `~auto` 的默认完成态/)
  assert.match(auto, /不得把“未执行修改”“等待下一步确认”写成 `✅完成` 的下一步/)
  ...
  assert.match(plan, /只有在 `~auto` 内触发 `~plan` \/ `~design` 语义时/)
  assert.match(plan, /显式 `~plan` 或 `~design`/)
  assert.match(plan, /不得用 `✅完成` 表示“未执行修改”/)
  ...
  assert.match(readText(join(REPO_ROOT, 'skills', 'commands', 'loop', 'SKILL.md')), /`🔄 下一步` 不能作为单轮结果或继续执行占位/)
  assert.match(readText(join(REPO_ROOT, 'skills', 'commands', 'loop', 'SKILL.md')), /必须写结构化 `turn-state` blocker/)
