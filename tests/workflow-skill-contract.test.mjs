...
test('workflow skill contracts stay aligned with command aliases and artifacts', () => {
  ...
  assert.match(help, /纯标准模式、且项目未初始化时不会自动触发这些技能/)
  assert.match(help, /UI 质量基线约束/)
  assert.match(help, /0=关闭\/1=知识库已存在时自动同步\/2=编码任务在知识库已存在或全局模式下自动创建或同步/)
  ...
  assert.match(readme, /initialized projects/)
  assert.match(readme, /project-level global-mode marker/)
  assert.match(readme, /syncs knowledge automatically only when the KB already exists/)
  assert.match(readme, /for coding tasks, auto-creates or syncs the KB/)
  ...
  assert.match(readmeCn, /已初始化项目/)
  assert.match(readmeCn, /项目级全局模式标记/)
  assert.match(readmeCn, /仅在知识库已存在时自动同步/)
  assert.match(readmeCn, /编码任务在知识库已存在或全局模式下自动创建或同步/)
  ...
  assert.match(init, /仅创建、补全或同步项目知识库/)
  assert.match(init, /`~init` 与 `~wiki` 同义/)
  assert.match(init, /目录结构、模板格式和状态文件重写规则按当前已加载的 HelloAGENTS 规则执行/)
  assert.match(init, /不写入项目级规则文件，也不创建项目级 HelloAGENTS 包根链接/)
  assert.doesNotMatch(init, /项目根载体/)
  assert.match(init, /初始“主线目标”写当前知识库初始化 \/ 同步任务/)
  assert.match(init, /只记录当前知识库任务/)
  assert.doesNotMatch(init, /HELLOAGENTS_PROFILE: full/)
  assert.doesNotMatch(init, /读取 `\{插件根目录\}` 中的全量规则模板/)
  assert.doesNotMatch(init, /各宿主项目级 HelloAGENTS 包根链接/)
  assert.doesNotMatch(init, /\.claude\/skills\/helloagents/)
  assert.doesNotMatch(init, /\.gemini\/skills\/helloagents/)
  assert.doesNotMatch(init, /\.codex\/skills\/helloagents/)

  const globalSkill = readText(join(REPO_ROOT, 'skills', 'commands', 'global', 'SKILL.md'))
  assert.match(globalSkill, /初始化项目级全局模式/)
  assert.match(globalSkill, /不受 `kb_create_mode` 限制/)
  assert.match(globalSkill, /项目级规则文件和各宿主项目级 HelloAGENTS 包根链接/)
  assert.match(globalSkill, /HELLOAGENTS_PROFILE: full/)
  assert.match(globalSkill, /各宿主项目级 HelloAGENTS 包根链接会刷新/)
  assert.match(globalSkill, /AGENTS\.md/)
  assert.match(globalSkill, /CLAUDE\.md/)
  assert.match(globalSkill, /\.gemini\/GEMINI\.md/)
  assert.match(globalSkill, /\.claude\/skills\/helloagents/)
  assert.match(globalSkill, /\.gemini\/skills\/helloagents/)
  assert.match(globalSkill, /\.codex\/skills\/helloagents/)
  ...
  assert.match(commit, /1 = 知识库已存在时自动同步（默认）/)
  assert.match(commit, /2 = 编码任务在知识库已存在或全局模式下自动创建或同步/)
