...
test('workflow skills stay aligned with the new qa-review architecture', () => {
  ...
  assert.match(auto, /~ask \/ ~plan \/ ~build \/ ~qa \/ ~prd/)
  ...
  const ask = readText(join(REPO_ROOT, 'skills', 'commands', 'ask', 'SKILL.md'))
  assert.match(ask, /交互式需求澄清/)
  assert.match(ask, /不创建 `\.helloagents\/`/)
  assert.match(ask, /一问一答/)
  assert.match(ask, /优先.*选择/)
  assert.match(ask, /升级路径/)
