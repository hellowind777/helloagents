...
test('workflow skill contracts stay aligned with command aliases and artifacts', () => {
  ...
  assert.match(plan, /`\.helloagents\/STATE\.md` 当恢复快照使用/)
  ...
  assert.match(build, /`\.helloagents\/STATE\.md` 作为恢复快照/)
  ...
  assert.match(prd, /`\.helloagents\/STATE\.md` 当恢复快照使用/)
  ...
  assert.match(init, /只作为当前初始化链路的恢复快照/)
  ...
  assert.match(readText(join(REPO_ROOT, 'skills', 'commands', 'loop', 'SKILL.md')), /强制恢复快照/)
