...
test('README and bootstrap docs expose qa-review instead of the old split review+verify path', () => {
  ...
  assert.match(bootstrap, /涉及判断与取舍时，先判断约束是否真实，再给干净目标，最后再谈迁移路径；公开 API、持久化数据、已文档化集成、用户承诺、部署与合规要求等才算真实约束/)
  ...
  assert.match(bootstrapLite, /涉及判断与取舍时，先判断约束是否真实，再给干净目标，最后再谈迁移路径；公开 API、持久化数据、已文档化集成、用户承诺、部署与合规要求等才算真实约束/)
