...
- 修复 Cursor 标准模式钩子为空：兼容扁平格式，正确写入通知钩子；标准模式增加安装提示
- 修复 Grok 与 Hermes 基础钩子被附加组件覆盖：改为合并写入，仅增删附加组件条目，基础条目保持不动
- 修复 Hermes 卸载残留空配置：无用户内容时删除文件，有用户内容时完整保留
- 修复 Codex 市场索引卸载残留：插件为空时删除文件
- 运行副本补齐 `.codex-plugin`，npm 包补齐 `README.md` 与 `LICENSE.md`；包描述补齐全部六个宿主
- 移除受管条目后不再留下空钩子文件，无其他内容时直接删除
- 帮助补齐 Codex 的危险命令拦截支持；更新时的远端拉取提示补齐多语言；一键安装脚本固定为 UTF-8 输出
- 修复命令路由：短写与全称使用同一解析，未知命令不接管，技能路径统一指向用户级运行副本，保留旧版别名
- 安装验证改为核对当前完整内核，避免提示词措辞调整导致误报；补充命令路由边界回归测试
- 保持严格 checkJs 检查，补齐安装、配置和 hook 模块的 JSDoc 类型与索引边界；修复安装来源读取丢失及运行时参数转发。Git 来源更新仅允许干净工作区同分支快进，不再强制重置本地内容
- 校正初始化与 CI 文档，明确接口兼容性和审查验证条件，补充只读敏感信息处理、无回收站时的保留策略及工具界面的验收边界
