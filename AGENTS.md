...
COMPACTION RECOVERY: If context was compressed during the session (previous messages were summarized, not at session start), immediately read {KB_ROOT}/plan/*/tasks.md (specifically LIVE_STATUS section) to restore workflow state (all G6-defined state variables: workflow variables, task complexity variables, knowledge base and package variables) and review the compression summary for key decisions, analysis results, and user preferences from prior conversation. Combine restored state and conversation substance with current user input to determine actual current state and correct next action, avoiding incorrect re-evaluation or stage confusion. Continue from interruption point if workflow should proceed (user input is task-related or continuation request), or enter routing if user requests new task.
...
```yaml
优先级: {CWD}/.helloagents/config.json > {HELLOAGENTS_ROOT}/config.json > 以上默认值
加载: 按 G7 会话启动规则静默读取，不存在→静默跳过，使用默认值
格式: {"CSV_BATCH_MAX": 0, "EVAL_MODE": 2}  # 仅列出需要覆盖的键
作用域: {HELLOAGENTS_ROOT}/config.json = 全局（所有项目）| {CWD}/.helloagents/config.json = 当前项目
...
| 检查点 | 触发时机 | 执行范围 | 职责 |
|--------|---------|---------|------|
| 路由初筛 | G4 级别判定时 | 仅第一层（关键词） | 快速判定是否需要强制 R3；不执行语义分析 |
| 执行前完整检查 | R1/R2/R3 进入执行前 | 三层全执行 | 完整风险评估，首次产出 EHRB 结论 |
| 确认前复核 | R3 阶段二（第 479 行） | 引用执行前结论 | 仅在执行前检查后有新增信息时重新检查；无新信息则直接引用 |
...
级别判定（单次判定，逐维度评估，取最高级别）:
  维度（未明确指定 = 未知，不可假设已知）:
    ...
    EHRB: 路由判定时初筛命中（仅关键词层）→ 强制 R3 [→ G2 EHRB 检查点职责分离]
  判定规则:
    - 任一维度命中 R3 → 整体为 R3
...
维度评分标准（CRITICAL - R2 和 R3 共用，逐维度独立打分后求和）:
  ...
  打分规则（CRITICAL）:
    ...
    - 打分信息来源（按优先级）:
      ...
      3. 任务类型的公认行业惯例（如棋类游戏隐含棋盘+回合制、浏览器游戏隐含 Web 环境）→ 该维度计 1 分并标注"类型惯例"，不得超过 1 分
    - 以上三类来源之外的信息 = 0 分。不得凭主观判断推测用户未表达的意图。
...
R2 评估流程（单回合，问题与确认合一）:
  1. 需求理解 + 逐维度打分（已有项目时须读取项目上下文辅助理解；新建项目可读取参考上下文）
  2. EHRB 检测 [→ G2]
...
```yaml
活状态存储: 方案包目录下 .status.json（独立 JSON 文件，取代旧的 tasks.md HTML 注释）
  格式: {"status": "in_progress", "completed": N, "failed": N, "pending": N, "total": N, "done": N, "percent": N, "current": "当前任务描述", "updated_at": "YYYY-MM-DD HH:MM:SS"}
  工作流变量: G6 定义的状态变量仍由 AI 在上下文中维护，.status.json 仅记录任务进度
  向后兼容: tasks.md 中的 <!-- LIVE_STATUS --> 注释保留为检测标记，实际数据读写使用 .status.json
更新时机: 任务开始、状态变更、遇到错误、阶段切换、流程/模式变更（如 ~auto 切换委托、R1→R2 升级）
状态恢复: 缺少上下文时，读取 .status.json + tasks.md 恢复进度
```
