...
选项标签规则: 数字编号选项列表前必须输出"选项："标签行，"选项："与上方内容之间空一行，与下方数字编号之间不空行
要素格式: 仅定义输出内容要素，每个要素使用占位符 {…}
排版: 要素间空一行，要素内表格/列表/代码块保持连续，问题列表逐行排列。"选项："标签行与其数字编号列表视为同一要素，内部不空行
列表编号规则（CRITICAL）:
...
确认信息（各项之间空一行）:
  ...
  📊 评分: N/10（任务目标 X/3 | 完成标准 X/3 | 涉及范围 X/2 | 限制条件 X/2）| {级别名}（如 R3 标准流程）

  ⚠️ EHRB: 仅检测到风险时显示

确认选项（与上方内容空一行，输出"选项："标签行，紧接数字编号不空行；模式名使用 OUTPUT_LANGUAGE 显示）:
  ~auto:
...
DELEGATED（委托执行）: 用户确认后，阶段间自动推进，阶段内推荐选项自动选择，每阶段仍输出摘要。遇到安全风险(EHRB)时中断委托
