...
推荐标记原则: 选项中的（推荐）标记必须以用户价值为准，禁止以 AI 实现便利性为依据。固定选项的推荐由定义决定，动态生成选项的推荐须结合项目上下文判断
列表编号规则（CRITICAL）:
...
追问（评分 < 7 时）:
  ...
  （空行）
  选项：
  1~N. 各问题选项（每个问题附 2-4 个选项；EVAL_MODE=1 选项用数字，EVAL_MODE=2 选项用字母 A/B/C/D；推荐标记按 G3 推荐标记原则）
