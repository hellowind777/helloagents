...
// Zero dependencies, ES module, cross-platform
...
function resolveWav(event) {
  const pkgPath = join(PKG_ROOT, 'assets', 'sounds', `${event}.wav`);
  if (existsSync(pkgPath)) return pkgPath;
  return null;
}

function playSound(event) {
  const wav = resolveWav(event);
...
function desktopNotify(event, extra) {
  ...
  try {
    if (PLAT === 'win32') {
      ...
      const iconPath = join(PKG_ROOT, 'assets', 'icons', 'icon.png').replace(/\//g, '\\');
...
`.trim();
      spawnSync('powershell', ['-NoProfile', '-c', ps],
        { stdio: 'ignore', windowsHide: true });
    } else if (PLAT === 'darwin') {
...
function cmdCodexNotify() {
  ...
  } else if (type === 'agent-turn-complete') {
    const lastMsg = data['last-assistant-message'] || '';
    const settings = readSettings();

    // Filter subagents: when output_format is enabled (default), only main agent
    // includes 【HelloAGENTS】 marker. When disabled, skip filter (accept limitation).
    const useFormat = settings.output_format !== false;
    if (useFormat && !lastMsg.includes('【HelloAGENTS】')) return;

    // Only run verification when AI claims task completion
    const claimsComplete = /✅|完成|已修复|done|fixed|completed|finished/i.test(lastMsg);

    // Ralph Loop verification (only on completion claims)
    const cwd = data.cwd || process.cwd();
    let verifyFailed = false;

    if (claimsComplete && settings.ralph_loop_enabled !== false) {
      try {
        const rlPath = join(__dirname, 'ralph-loop.mjs');
        if (existsSync(rlPath)) {
          const result = spawnSync(process.execPath, [rlPath], {
          ...
          if (result.stdout) {
            try {
              ...
              if (rlOut.decision === 'block') {
                verifyFailed = true;
                playSound('warning');
                desktopNotify('warning', `验证未通过: ${(rlOut.reason || '').slice(0, 80)}`);
              }
            } catch {}
          }
        }
