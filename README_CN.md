...
HelloAGENTS 还在 `bootstrap.md` / `bootstrap-lite.md` 这层默认启用一组常驻核心规则：先区分真实外部契约与内部惯性，再决定是否保留旧行为；先给干净目标，再谈迁移路径；大胆方向必须补首个证明点、证伪条件与止损规则。用户可见文本默认只使用当前回复语言，除代码标识、命令、文件名、目录名、路径、标记名、配置键和必要专名外，避免中英文混杂。
