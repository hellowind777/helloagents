...
- **Claude Code** — 注入规则到 `~/.claude/CLAUDE.md`，配置 hooks 和 skills 符号链接
- **Gemini CLI** — 注入规则到 `~/.gemini/GEMINI.md`，配置 hooks
- **Codex CLI** — 配置 `config.toml`、hooks 和 skills 符号链接

这是**标准模式**（默认）— 所有项目注入精简规则，在项目中使用 `~init` 激活完整功能。

### 全局模式（插件安装）

如需所有项目启用完整规则，切换到全局模式并安装插件：
...
| **全局模式** | 各 CLI 安装插件/扩展 | `bootstrap.md`（完整规则） | 14 个技能自动激活 | 全面使用 HelloAGENTS |

标准模式直接注入规则到 CLI 配置文件（`~/.claude/CLAUDE.md`、`~/.gemini/GEMINI.md`、`~/.codex/config.toml`）并创建 skills 符号链接。全局模式使用各 CLI 原生的插件/扩展系统。
