...
- 全局模式会自动尝试安装 Claude Code 插件和 Gemini 扩展；Codex 继续使用受管本地插件流程，并刷新 marketplace 与缓存副本。
- `~auto` 和 `~loop` 的结构化停顿门控更严格；流程确需暂停时，必须写明具体阻塞证据。
- 运行态状态写入 `.helloagents/sessions/<branch>/<session>/` 下的会话胶囊，分别记录 turn-state、事件和交付证据，兼容多 CLI 窗口与多 worktree。
