...
- **Feedback-Delta 机制**：基于语义理解而非关键词匹配，识别增量反馈并在当前阶段迭代，避免不必要的阶段切换
...
- **G7 产品设计原则**：新项目/新功能时，从产品经理视角进行深度需求分析，关注用户研究、可行性评估、人文关怀设计
- **G8 安全与合规**：EHRB 识别与自动缓解，禁止外联生产与明文秘钥
- **G9 大型项目策略**：渐进式知识库初始化、任务分解（单任务 50-100 行）、分级测试策略
...
- **P4｜错误处理（按需）**：MRE 复现 → 修复 → 复盘与文档同步；优先怀疑代码实现错误；**双层保护机制**：①针对同一错误连续失败≥2次警告、≥3次强制暂停；②针对P4循环（≥4次或连续不同错误）主动询问是否回到P1/P2重新评估。
- **Feedback-Delta**：基于语义理解识别增量反馈（指向性语义 + 增量修改意图 + 时间连续性 + 非跨阶段指令），在当前阶段迭代而非重新路由。
...
## 版本与升级
2025-11-10.10版本更新：
* **章节编号规范化**：修正全局规则章节编号（G7 产品设计原则、G8 安全与合规、G9 大型项目策略）
* **新增 G7 产品设计原则**：针对新项目/新功能，从产品经理视角进行深度需求分析，包含用户研究、可行性评估、体验设计、包容性设计、道德考量
* **Feedback-Delta 规则优化**：强调基于语义理解而非关键词匹配，采用四维判定条件（指向性语义、增量修改意图、时间连续性、非跨阶段指令）
* **P4 错误判定标准精确化**：采用语义化根因判定，而非仅依赖错误消息字面匹配
* **P1/P2 阶段增强**：新增产品视角分析和产品方案设计步骤（触发 G7 时执行）
* **知识库模板完善**：HELLOWIKI.md 新增产品设计章节，包含核心价值主张、用户画像、用户旅程地图、人文关怀设计、成功指标

2025-11-10.09版本更新：
* P4 迭代保护机制全面升级，引入双层保护机制
* 知识库管理策略细化
* Feedback-Delta 触发条件补充

2025-11-10.08版本更新：
* **P4 迭代保护规则优化**：移除"重建知识库"的误导性表述，明确为三个具体应对方向：
  1. 全面重新扫描代码库（可能遗漏关键模块/配置/依赖）
  2. 回到P1重新分析（可能根因判断错误）
  3. 回到P2重新制定方案（可能修复策略根本错误）
* P4连续执行≥3次时改为强制暂停并要求用户介入决策，需说明已尝试的方向、可能的根本原因、建议的下一步
...
* 新增 **P4 迭代保护**：连续 ≥2 次未解决警告，≥3 次强制提示
