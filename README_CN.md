...
- CCswitch 用户注意：CCswitch 工具会自动保存和恢复 CLI 设置文件（如 `settings.json`、`config.toml` 等）。如果 HelloAGENTS 的配置（hooks、permissions、rules 等）在卸载前已被 CCswitch 保存，卸载 HelloAGENTS 后切换 CCswitch 配置方案会导致这些配置被重新加载回来。请在卸载 HelloAGENTS 后自行清理 CCswitch 中已保存的相关配置内容
...
- AGENTS.md 从 1186 行精简至 ~700 行（-41%），降低大模型选择性忽略风险
- G4 评估详情、G9/G10 子代理协议下沉为独立模块文件，通过 G7 按需加载
- 子代理规则文件（subagent.md）从 26KB 降至 1.4KB（-94%）
- 消除跨模块引用风险，关键规则内联到使用位置
- inject_context.py 增加模块加载信号，保障上下文压缩后规则可恢复
...
- 动态子代理并行数量，消除硬编码限制
- 统一输出格式与精简执行路径
- Codex CLI 内存限制调整至 128 KiB，防止规则文件被截断
- 优化推荐选项生成规则、方案差异化要求和评估评分标准
