...
插件自动加载 `bootstrap.md` 规则、hooks 和全部 14 个质量技能。
...
`postinstall` 脚本自动配置 Codex CLI — 设置 `config.toml`、hooks 和 bootstrap 规则。
...
HelloAGENTS 支持两种安装模式：

| 模式 | 行为 | 适用场景 |
|------|------|----------|
| **标准模式** (默认) | 仅注入精简规则，项目需 `~init` 激活完整功能 | 按需使用，不影响其他项目 |
| **全局模式** | 所有项目自动启用完整规则 | 全面使用 HelloAGENTS |
...
**A：** Claude Code 体验最好（插件系统、11 个生命周期 hooks、Agent Teams 支持）。Gemini CLI 通过扩展系统支持。Codex CLI 也不错（npm postinstall 自动配置）。v3.0.0 不支持其他 CLI。
</details>
...
**A：** 标准模式（默认）仅注入精简规则，项目需要 `~init` 才能激活完整功能。全局模式自动为所有项目启用完整 HelloAGENTS 规则。通过 `helloagents --global` 或 `helloagents --standby` 切换。
</details>
...
- 检查 `~/.codex/config.toml` 中 `model_instructions_file` 是否指向 `bootstrap.md` 或 `bootstrap-lite.md`
- 重启 Codex CLI
