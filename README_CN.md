...
答：Role Language Model——HelloAGENTS 的子代理编排系统。包含 5 个专业角色（reviewer、synthesizer、kb_keeper、pkg_keeper、writer）+ 原生 CLI 子代理。任务通过 DAG 依赖分析调度，可并行时自动并行。详见[使用指南](#使用指南)。

**问：项目知识存储在哪里？**

答：在项目本地的 `.helloagents/` 目录中。知识库在代码变更时自动同步（由 `KB_CREATE_MODE` 配置控制）。包含模块文档、CHANGELOG、会话摘要和自定义命令。详见 [KB 自动同步与 CHANGELOG](#kb-自动同步与-changelog)。

**问：记忆能跨会话保留吗？**

答：能。通过三层记忆：L0 用户记忆（全局偏好，存于 `~/.helloagents/`）、L1 项目知识库（按项目存于 `.helloagents/`）、L2 会话摘要（阶段转换时自动保存）。即使关闭重开 CLI，上下文依然保留。
