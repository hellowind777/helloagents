...
在全局模式、项目已写入 full project carrier 或明确的 UI 工作流里，`hello-ui` 会在该基线之上补充设计契约执行、设计系统映射与视觉验收。
...
| `~wiki` | 只创建或同步项目知识库 |
| `~init` | 完整项目初始化：知识库、full 项目级 carrier 和包根链接 |
| `~test` | 为指定模块或最近变更编写测试 |
...
`~init` 做得更多：创建或更新知识库、写入 full 项目级 carrier，并刷新各宿主项目级 HelloAGENTS 包根链接。
...
### full carrier 与 lite carrier

标准模式下，未写入 full 项目级 carrier 的项目只获得轻量规则和显式 `~command` 入口。执行 `~init` 后，项目级 carrier 会写入 `<!-- HELLOAGENTS_PROFILE: full -->`，项目才进入 full 项目工作流。
...
|------------|------|
| `~wiki` | 只创建或同步知识库 |
| `~init` | 创建知识库，同时写入 full 项目级 carrier 和包根链接 |
| `kb_create_mode = 0` | 关闭自动知识更新 |
| `kb_create_mode = 1` | full carrier 项目或全局模式中，编码任务自动更新知识 |
| `kb_create_mode = 2` | full carrier 项目或全局模式中，更积极地更新知识 |
...
还想写入 full 项目级 carrier 和宿主项目级 HelloAGENTS 包根链接，用 `~init`。

### standby 和 global 有什么区别？

`standby` 更轻量、更显式。它只把规则部署到指定 CLI，完整项目流程由 full 项目级 carrier 触发。
