...
- 宿主提供稳定会话标识时：`.helloagents/sessions/<workspace>/STATE.md`
- 宿主未提供稳定会话标识时：`.helloagents/sessions/<workspace>/STATE.md`

`<workspace>` 是当前 Git 分支、detached HEAD 的 `detached-<sha>`，或非 Git 项目的 `workspace`。`.helloagents/sessions/active.json` 只记录当前活跃工作区指针。
...
`STATE.md` 只保留给人看的恢复快照。`turn-state`、路由上下文和 artifact 索引改写到同目录的 `runtime.json`；`events.jsonl` 仍是可选 trace 输出，默认不写。
...
一旦任务会创建或修改本地文件，或会在当前项目留下本地输出，HelloAGENTS 就会自动创建项目本地 `.helloagents/sessions/<workspace>/STATE.md`，而不是只停留在用户级临时运行态。
