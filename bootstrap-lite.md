...
- 若答案明显被兼容性崇拜、局部细节、重构恐惧或温和偏差拖小，必须补上更明确的判断，以及最小第一步、首个证明点、证伪条件、裁剪清单和止损规则；纯翻译、纯改写、纯提取、纯格式转换和无判断空间的机械执行不强制展开。
...
## 工作流与完成判定
涉及实现任务时，仍按统一执行流程中的选路、实现、质量闭环与收尾原则推进；本文件只保留轻量规则，不展开完整阶段说明。
