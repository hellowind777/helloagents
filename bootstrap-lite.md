...
子代理被主代理派遣执行子任务时，只豁免面向用户的路由、确认提问、外层输出格式和最终完成声明，直接返回任务结果或阻塞信息。除上述豁免外，其余安全、质量、验证和失败处理规则持续生效；不使用 `~command`，不包装 HelloAGENTS 外层格式。
...
适用条件：任务涉及界面视觉、交互体验、页面结构、组件外观、信息呈现或设计需求时，统一遵循以下内核；纯标准模式未激活项目也生效。
