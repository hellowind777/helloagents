...
创建:
  目录结构: 按 G1 知识库完整结构创建（含 sessions/, user/ 目录）
  模板: 读取 services/templates.md
  项目级用户偏好: 创建 user/ 目录（空目录，用户按需添加 .md 文件覆盖全局偏好）
  大型项目（TASK_COMPLEXITY=complex）: 按编排五步法调度子代理并行扫描不同模块目录（强制）[→ G10 调用通道]
    编排: 按模块目录拆分，每个子代理负责扫描一个目录（子代理数=模块目录数，≤6/批）
    每个子代理 prompt 明确: 负责的目录路径 + 扫描内容（文件结构/入口点/导出接口/依赖关系）
    主代理汇总各子代理扫描结果后统一创建
  全局记忆: 检查 {HELLOAGENTS_ROOT}/user/memory/ 目录，不存在时自动创建
```
