...
| 模式 | 特点 | 流程 | 子代理 |
|------|------|------|--------|
| R2 适度 | 跳过多方案对比 | → 步骤5 | designer（按需） |
| R3 标准 | 复杂任务多方案对比 | → 步骤2-7 | analyzer（强制并行） |
...
项目规模判定: 按 G9 complex 级别评估，影响任务拆分粒度
```
...
```yaml
R3 标准: 生成2-3个方案 → 子代理详细评估 → 确定推荐方案
  评估标准: 优缺点、性能、可维护性、复杂度、风险(含EHRB)、成本、最佳实践
  INTERACTIVE → 输出方案对比 → ⛔ END_TURN（等待用户选择方案N/重新构思/取消）
...
填充步骤:
  ...
  2. 处理执行报告
  3. 填充 proposal.md（按场景深度）
  4. 填充 tasks.md（元数据头部+任务清单+状态符号）
  5. 设置 CREATED_PACKAGE
```
...
失败处理:
  INTERACTIVE → 输出: 警告（验证详情）→ 修复/强制继续/终止
  DELEGATED:
