...
子代理调用（按步骤3 TASK_COMPLEXITY）:
  moderate/complex + 现有代码库 → 调度原生子代理执行代码库扫描和结构分析（强制）[→ G10 调用通道]
    按模块目录数分配子代理并行扫描（子代理数=目录数，≤6/批），主代理汇总结果
...
子代理调用:
  complex+依赖>5模块 → 调度原生子代理执行深度依赖分析和质量评估（强制）[→ G10 调用通道]
    按涉及模块数分配子代理并行分析（子代理数=模块数，≤6/批），主代理汇总结果
  其他 → 主代理直接执行
...
```yaml
R3 标准流程（CRITICAL）:
  方案构思: 调度 ≥3 个原生子代理（按需求复杂度决定数量），每个子代理独立构思一个差异化方案（并行）[→ G10 调用通道]
