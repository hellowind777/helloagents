...
| 模式 | 特点 | 流程 | 子代理 |
|------|------|------|--------|
| R2 简化流程 | 跳过多方案对比 | → 步骤5 | 原生子代理执行方案设计 R2 moderate/complex 强制 [→ G10 调用通道] |
| R3 标准流程 | 复杂任务多方案对比 | → 步骤2-7 | 原生子代理执行方案评估 R3+候选方案≥2 强制 [→ G10 调用通道] |
...
原生子代理(评估) → 原生子代理(规划) → [RLM:synthesizer](综合) → [RLM:pkg_keeper](填充)
每个环节的输入依赖前一个环节的输出，禁止并行调度，逐个调用等待返回后再发下一个

```yaml
R3 标准流程: 生成2-3个方案 → 原生子代理详细评估（强制）[→ G10 调用通道] → 确定推荐方案
