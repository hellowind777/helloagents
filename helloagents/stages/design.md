...
```yaml
加载: stages/analyze.md [阻塞式]
执行: analyze.md 步骤1-6（KB检查、复杂度评估、项目上下文获取、技术分析）
产出: KB_SKIPPED、TASK_COMPLEXITY、项目上下文
完成后: 输出上下文收集摘要，继续 Phase2
```
...
|------|------|------|--------|
| R2 简化流程 | 跳过多方案对比 | → 步骤5 | [RLM:pkg_keeper] 填充方案包（步骤5）强制 |
| R3 标准流程 | 复杂任务多方案对比 | → 步骤2-7 | 原生子代理评估（步骤4）+ [RLM:synthesizer] 综合（步骤4, complex时）+ [RLM:pkg_keeper] 填充（步骤5） |
...
⚠️ **串行强制:** DESIGN 阶段子代理存在严格数据依赖，禁止并行调度:
步骤4: 原生子代理(评估) → [RLM:synthesizer](综合, complex时)
步骤5: [RLM:pkg_keeper](填充)
步骤4 与步骤5 之间有用户选择（INTERACTIVE）或自动选择（DELEGATED），各段内部逐个调用等待返回后再发下一个

```yaml
R3 标准流程（CRITICAL）: 必须生成 2-3 个差异化方案（不得少于2个）→ 原生子代理详细评估（强制）[→ G10 调用通道] → 确定推荐方案
