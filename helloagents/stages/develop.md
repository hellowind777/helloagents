...
DIRECT入口（~exec，CURRENT_PACKAGE 已由 exec.md 设置）:
  ...
  CURRENT_PACKAGE 未设置（降级）:
    扫描 plan/ → 无方案包→输出: 错误 | 1个→设置 CURRENT_PACKAGE | 多个→输出: 确认（清单）
    多个时选项：
    1~N. 各方案包名称
    N+1. 取消
    ⛔ END_TURN
...
并行调度策略:
  ...
  职责隔离: 每个子代理的 prompt 必须明确其负责的函数/类/代码段，禁止职责范围重叠
  复杂任务拆分: 单个大任务可拆为多个子任务分配给不同子代理，同文件不同函数允许并行
  同文件并行安全: 各子代理只修改各自负责的函数/类体内代码，不得修改文件级共享区域（imports/模块变量/初始化逻辑），共享区域变更由主代理在汇总阶段统一处理
