...
影响:
  ...
  步骤11 CHANGELOG 始终执行
  TASK_COMPLEXITY: 决定步骤6/7子代理调度 + 步骤8验证范围（静态分析/构建/E2E） + 步骤9验收层级（功能/需求/体验）
```
  ...
  工具探测（步骤开始时，≤3秒）:
    ...
    视觉验证能力探测（UI 任务时必须执行）:
      按保真度从高到低探测，命中即停:
        ...
        框架级: 项目已配置的无头浏览器测试框架
        预览级: 纯 HTML/静态文件交付物 → 可用系统浏览器直接打开
        抓取级: 会话可用的 HTTP 抓取类工具 → 可获取渲染结果做结构验证
        代码级: 以上均无 → 步骤9 降级为代码审查
      探测结果供步骤9使用
...
体验验收（complex 全维度 | moderate + 交付物含 UI/API/CLI 时核心维度）:
  UI/前端（complex 全部，moderate 仅标★项）:
    ...
    ★视觉一致性: 有限统一的配色方案，统一圆角/阴影/间距体系，禁止浏览器原生无样式控件
    色彩/间距/字体规范性: 文字清晰可读（对比度≥4.5:1），留白充足不拥挤
    ★核心交互有反馈: hover/active 状态可感知，操作结果有视觉反馈（toast/状态变化/动画）
    动画流畅性 + 加载状态处理
    响应式适配 + ★错误状态UX（空状态/输入校验反馈）
    可访问性（img alt/aria-label/键盘导航/颜色对比度）
    质量底线（★，所有含UI任务强制检查）:
      - 无浏览器默认样式残留（所有可见控件均有自定义外观，明显区别于浏览器默认渲染）
...
验收透明度（moderate + complex，嵌入完成输出）:
  在验收报告中列出:
    ✅ 已验证项（方法+结果）
    ⚠️ 未验证项（原因）
    📋 建议手动确认项（具体操作步骤，如"在浏览器中缩小窗口至375px，检查导航栏折叠"）
  固定项（始终包含）:
