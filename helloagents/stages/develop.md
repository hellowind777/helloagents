...
体验验收（complex 全维度 | moderate + 交付物含 UI/API/CLI 时核心维度）:
  UI/前端（complex 全部，moderate 仅标★项）:
    ★布局合理性: 主内容区占据视觉主导地位（不被辅助元素挤压），控件按功能分组且间距均匀，无元素重叠或溢出
    ★视觉一致性: 有限统一的配色方案，统一圆角/阴影/间距体系，禁止平台默认无样式控件
    动画流畅性 + 加载状态处理
    质量底线（★，所有含 UI 任务强制检查）:
      - 无平台默认样式残留（所有可见控件均有自定义外观，明显区别于平台默认渲染）
      - 交互控件（按钮/输入框/菜单）有明确的视觉层级（主/次/危险操作可区分）
      - 排版层次可辨识（标题 > 正文 > 辅助文字有明确大小/粗细区分，正文 ≥ 16px/17pt/16sp，行高 ≥ 1.4 倍字号）
      - 间距体系一致（同级元素间距相等，嵌套层级间距递减，无视觉拥挤或过度留白）
      - 核心交互有反馈（Web: hover/active; 原生: pressed/highlighted 状态变化，操作结果有视觉反馈如 toast/状态变化/动画）
      - 错误状态 UX（空状态有提示、输入校验有反馈、操作失败有说明）
      - 响应式基线（主要内容在最小目标屏幕尺寸下不溢出不截断）
    质量底线验证参照: 对照 proposal.md §6 设计规格逐项核验（§6 存在时），建立 DESIGN→DEVELOP 闭环
    可访问性（moderate 及以上强制，simple 为建议项，按技术栈适配）:
      - 辅助功能标注覆盖（Web: img alt/aria-label; 原生: accessibility label/trait/hint）
      - 核心流程可通过键盘（Web）/手势导航（原生）完成
      - 文字与背景对比度 ≥ 4.5:1
  API/后端（complex 全部，moderate 仅标★项）:
    ★响应格式一致性 + 错误码规范 + ★核心边界输入处理
  CLI/工具（complex 全部，moderate 仅标★项）: ★帮助信息完整 + ★错误提示清晰 + 参数边界处理
  文档: 受众适配度 + 示例覆盖 + 术语一致性
  验证方式: 按步骤8探测的视觉验证能力级别执行对应的验证策略（参见步骤9功能验收的分级策略）
  发现问题 → ⚠️ 警告性，记录到验收报告
