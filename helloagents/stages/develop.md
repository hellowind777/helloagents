...
编程任务:
  工具探测（步骤开始时，≤3秒）:
    ...
    → 识别可用的验证能力（lint、类型检查、单元测试、E2E、构建、截图等）
...
功能验收（所有任务）:
  编程任务:
    有独立运行入口:
      有截图/浏览器工具（MCP/Skill/插件）+ UI任务 → 启动应用 + 截图验证核心页面
      无视觉工具 → 启动/运行交付物，执行核心交互，验证输出符合预期
...
体验验收（complex 全维度 | moderate + 交付物含 UI/API/CLI 时核心维度）:
  UI/前端（complex 全部，moderate 仅标★项）:
    ★布局合理性 + ★视觉一致性 + 色彩/间距/字体规范性
    ★核心交互有反馈 + 动画流畅性 + 加载状态处理
  ...
  文档: 受众适配度 + 示例覆盖 + 术语一致性
  验证方式: 有截图/浏览器工具 → 视觉验证（优先）| 无 → 代码级审查 + 标注"代码推断，未视觉验证"
  发现问题 → ⚠️ 警告性，记录到验收报告
