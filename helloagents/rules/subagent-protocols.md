...
角色清单: reviewer, synthesizer, kb_keeper, pkg_keeper, writer, brainstormer
Claude Code agent 文件（安装时部署至 ~/.claude/agents/）:
  reviewer → ha-reviewer.md | synthesizer → ha-synthesizer.md | kb_keeper → ha-kb-keeper.md
  pkg_keeper → ha-pkg-keeper.md | writer → ha-writer.md | brainstormer → ha-brainstormer.md
原生子代理映射（角色→类型映射，调用语法详见各 CLI 调用协议）:
  ...
  方案评估 → Codex: spawn_agent(agent_type="synthesizer") | Claude: Task(subagent_type="general-purpose") | OpenCode: coder | Gemini: generalist | Qwen: general-purpose
...
编排四步法:
  ...
  4. 汇总决策: 阻塞等待全部子代理返回后，主代理汇总实际返回内容并做最终决策 [→ 阻塞等待与结果真实性]
...
适用场景与编排策略:
  ...
  代码实现（功能开发/Bug修复/重构）:
    ...
    → 子代理类型: general-purpose / worker（Codex: RLM 角色使用各自 agent_type）
  方案构思（设计阶段多方案对比）:
    → 每个子代理独立构思一个差异化方案，不共享中间结果
    → 子代理类型: brainstormer（RLM 角色）— Codex: spawn_agent(agent_type="brainstormer") | Claude: Task(subagent_type="ha-brainstormer")
  质量检查（审查/验证/测试）:
    → 按分析维度拆分（质量/安全/性能），每个子代理负责≥1个维度
    → 子代理类型: general-purpose / worker（Codex: reviewer 等 RLM 角色使用各自 agent_type）
...
helloagents 角色:
  角色→agent_type 映射（每个 RLM 角色使用自己的 agent_type，注册在 config.toml [agents.{role}] 节）:
    ...
    synthesizer → spawn_agent(agent_type="synthesizer", prompt="...")
...
单次等待上限:
  每个子代理最多执行 2 轮等待（collab wait），第 2 轮仍 "No agents completed yet" → close → 降级
  DO NOT: 等待 3 轮及以上（每轮等待消耗上下文但无产出）
...
RLM 角色操作豁免:
  ...
  数据所有权范围（与 services/ 模块定义一致）:
    pkg_keeper: {KB_ROOT}/plan/*, {KB_ROOT}/archive/*
    kb_keeper: {KB_ROOT}/ 全部（除 plan/ 和 archive/，后者属于 pkg_keeper）
    reviewer: 只读，不修改文件
    synthesizer: 只读，不修改文件
...
子代理调用合规检查（阶段验收时执行）:
  TASK_COMPLEXITY=moderate/complex 时:
    DESIGN 阶段（含 Phase1 上下文收集）:
      检查: brainstormer 是否已调用（R3 标准流程步骤10 方案构思强制）
