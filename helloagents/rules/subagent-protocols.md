...
强制调用规则（标注"强制"的必须调用，标注"跳过"的可跳过）:
  ...
  DESIGN:
    Phase1（上下文收集）—
    子代理（按编排五步法选择类型）— moderate/complex+现有项目资源 项目资源扫描强制（步骤4）| complex+涉及>5个独立单元 深度依赖分析强制（步骤6）| simple 或新建项目跳过
    Phase2（方案构思）—
    brainstormer — R3 标准流程步骤10 方案构思时强制，≥3 个子代理并行（每个独立构思一个方案）
    方案包填充/知识库同步/归档 — 主代理按服务接口规范直接执行（不通过子代理中转）
  DEVELOP:
    子代理（按编排五步法选择类型）— moderate/complex 任务改动强制（步骤6，逐项调用）| 新增测试用例时强制（步骤8）| simple 跳过
    reviewer — complex+涉及核心/安全模块 强制 | 其他跳过
    知识库同步/归档 — 主代理按服务接口规范直接执行（不通过子代理中转）
  命令路径:
    ~review: 子代理（按编排五步法选择类型）— 审查文件>5 时各分析维度并行（质量/安全/性能，按复杂度分配子代理数量）
    ~validatekb: 子代理（按编排五步法选择类型）— 知识库文件>10 时各验证维度并行（按复杂度分配子代理数量）
    ~init: 子代理（按编排五步法选择类型）— complex 级别大型项目时模块扫描并行
...
通用并行信息收集原则（适用所有流程和命令）:
  ...
  ≥3个独立分析/验证维度 或 文件数>5 → 按编排五步法调度子代理并行执行
