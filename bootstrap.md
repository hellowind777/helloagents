...
会话级缓存优先：如果当前上下文中已包含"当前用户设置"、该配置文件的原始 JSON、或此前读取结果的摘要，且覆盖本次需要的配置项，视为配置已完成读取，直接复用。
只有当当前会话上下文缺少所需配置项、用户明确要求刷新配置、或本轮刚修改过该配置文件需要核验时，才读取一次配置文件；不要为了展示、确认、输出格式判定或同一结论重复读取。
输出格式判定属于受配置影响的行为；仅当当前会话上下文没有 `output_format` 的已知值时，最终收尾前才读取一次该配置。
