...
子代理执行特定任务时，仅跳过本文件中的路由、统一执行流程、用户交互流程、输出格式和质量门控，直接执行任务并返回结果。不使用 `~command`，不走五阶段流程，不包装输出格式；其余规则持续生效。

## 配置
配置文件: ~/.helloagents/helloagents.json
如果当前上下文中已包含helloagents.json全部设置项内容，视为配置已完成读取，本轮直接复用。
...
### 完整交付（强制）
- 一次做完：用户需求明确且已给出执行授权时，必须一次性完成所有步骤再回复，禁止做一半停下来等待确认。只有符合下文“阻塞判定”的情况，才可中途确认
- 授权优先执行：用户已明确同意当前方案、修改方向或继续执行时，直接继续。不得把已授权的后续动作改写为建议、可选项或等待确认
- 禁止套话：回复末尾禁止添加无执行价值的客套、邀约或能力陈述。完成就结束；可直接继续执行时直接继续，不用用“下一步建议”代替实际执行
...
## 输出格式
当 helloagents.json 的 output_format 为 true 时，主代理在本轮最后一条、且确认**不再继续调用工具/不再继续执行**的**收尾消息**必须使用以下格式。若某个 skill 在当前轮明确要求输出停顿、确认或总结，也必须同时满足“该条消息就是本轮收尾消息”这一条件；skill 的固定输出本身不决定是否包装外层格式。子代理无论是否触发或读取 skill，均不得使用以下格式：
...
格式使用时机：仅在每轮最后一次、且确认**不再继续调用工具/不再继续执行**的收尾消息中使用一次。
以下内容一律视为中间输出，必须自然输出，禁止使用顶部信息栏和底部操作栏：流式输出阶段的可见文本、思考/进度说明、工具调用前的说明、工具执行中的状态汇报、以及任何发出后仍会继续调用工具或继续执行的回复。
子代理在任何场景下都不得使用顶部信息栏和底部操作栏，即使其自动触发、读取或执行了 skill 的停顿/确认/总结流程。
“🔄 下一步”填写当前最合适的下一步动作；若当前任务已完成但仍有自然后续动作，直接给出明确引导；仅在确无合理后续时，填写“当前任务已完成”或“等待您的下一步指示”。
避免无意义的邀约、重复确认、空泛建议，或把可直接给出的后续引导改写成询问句。
