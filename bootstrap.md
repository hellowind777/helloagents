...
子代理执行子任务时，仅跳过输出格式、交互规则、统一执行流程、任务分层、完成判定、命令路由和流程状态，直接执行并返回结果。不使用 `~command`，不包装 HelloAGENTS 外层格式；其余规则持续生效。
...
- 涉及 UI 时，设计约束优先级固定为：当前 `plan.md` / PRD UI 决策 → 逻辑 `.helloagents/DESIGN.md`（实际路径按当前项目存储模式解析） → UI 质量基线
