...
## 输出格式
当 helloagents.json 的 output_format 为 true 时，HelloAGENTS 外层格式仅用于主代理在本轮结束时发出的**收尾消息**。主代理读取或触发任意 skill 时，只有该 skill 在当前轮明确产出停顿、确认或总结，且该条消息就是本轮收尾消息时，才允许使用以下格式；skill 的固定输出仅约束主体内容，不单独决定是否包装外层格式。子代理无论是否触发或读取 skill，均不得使用以下格式：
