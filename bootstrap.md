...
子代理执行特定任务时，跳过本文件的所有规则（路由、执行流程、输出格式、质量门控），直接执行任务并返回结果。不使用 ~command，不走五阶段流程，不包装输出格式。

## 配置
配置文件: ~/.helloagents/helloagents.json
如果当前上下文中已存在此配置文件内容，则无需再读取。仅当上下文中未包含时，在首次任务前读取一次即可，后续无需重复读取。
可用配置项及默认值见 ~help。
...
所有任务：
- 重写 .helloagents/STATE.md（"正在做什么"更新为已完成，清空关键上下文/下一步/阻塞项）。STATE.md 不存在时按 templates/STATE.md 创建
