...
如果你是被派遣执行特定任务的子代理，跳过此 skill 的路由与流程要求，直接执行任务；但基础安全、质量与失败处理规则仍持续生效，且无论是否触发或读取其他 skill，都不得包装 HelloAGENTS 外层输出格式。
如果你是主代理并触发或读取任意 skill，只有当该 skill 在当前轮明确要求输出停顿、确认或总结，且该条消息就是本轮收尾消息时，才按当前已加载 bootstrap 规则包装外层格式；流式输出、工具调用前/中的中间文本，以及任何仍将继续执行的文本，均不属于格式包装范围。skill 的固定输出只约束主体内容，不单独决定是否包装外层格式。
