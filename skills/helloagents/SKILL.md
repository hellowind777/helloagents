...
主代理在本轮最终收尾前，如要报告“已完成 / 等待输入 / 本轮阻塞”，必须先调用 `node "{HELLOAGENTS_READ_ROOT}/scripts/turn-state.mjs" write` 写机器可读的 turn-state；不要让运行时再从自然语言或图标反推状态。`turn-state.mjs` 是可执行收尾脚本，普通对话、选路和收尾不要读取源码；只有用户明确要求分析、修改该脚本，或正在排查运行时问题时才按需读取。若写 `kind=waiting` 或 `kind=blocked`，还必须同时写 `reasonCategory` 与 `reason`。子代理不得写 turn-state。
