...
- `~design` 是 `~plan` 的兼容别名，显式 `~design` 与显式 `~plan` 一样默认停在可执行方案；只有在 `~auto` 内触发 `~plan` / `~design` 语义时，方案包写入后才默认继续进入 `~build`
...
如果当前任务是显式 `~plan` 或 `~design`，且尚未获得执行授权，收尾必须使用 `❓等待输入`，下一步写清“等待确认是否按方案进入执行”，不得用 `✅完成` 表示“未执行修改”。
