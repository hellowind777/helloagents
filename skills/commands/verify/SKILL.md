...
0. 先对齐当前工作流状态：
   ...
   - 若 `contract.json` 声明 `advisor.required=true`，则本轮还必须补齐 `.helloagents/.ralph-advisor.json`；advisor 是可选能力，不是默认常驻步骤
1. 先决定验证分流：
...
3. 全量验证模式或审查后继续验证：
   ...
   - 若 `advisor.required=true`，在进入收尾前调用 `scripts/advisor-state.mjs write` 写 `.helloagents/.ralph-advisor.json`；记录触发原因、focus、consultedSources、结论与建议，禁止只在自然语言里留一段 advisor 意见
4. 汇总报告：
   - ✅ 通过的审查项 / 命令
   - ❌ 失败的审查项 / 命令 + 错误详情
