...
| output_format | true | true=仅主代理的最终收尾回复可使用 HelloAGENTS 格式；若由 skill 产出停顿/确认/总结，也必须同时是本轮收尾消息；流式/中间输出及所有子代理输出保持自然；false=自然输出 | Claude Code + Gemini CLI + Codex CLI |
